...
function woocommerce_edit_order_columns($columns){
	...
	$columns["order_status"] = __("Trạng thái", 'woocommerce');
	$columns["order_title"] = __("Đơn hàng", 'woocommerce');
	$columns["billing_address"] = __("Địa chỉ thanh toán", 'woocommerce');
	$columns["shipping_address"] = __("Địa chỉ giao hàng", 'woocommerce');
	$columns["total_cost"] = __("Tổng đơn hàng", 'woocommerce');
	$columns["order_comments"] = '<img alt="' . esc_attr__( 'Ghi chú', 'woocommerce' ) . '" src="' . $woocommerce->plugin_url() . '/assets/images/order-notes_head.png" class="tips" data-tip="' . __("Order Notes", 'woocommerce') . '" width="12" height="12" />';
	...
	$columns["order_date"] = __("Ngày", 'woocommerce');
	$columns["order_actions"] = __("Thực hiện", 'woocommerce');
