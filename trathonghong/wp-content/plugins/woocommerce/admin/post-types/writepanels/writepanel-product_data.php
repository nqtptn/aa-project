...
function woocommerce_product_data_box() {
	...
	$product_type_selector = apply_filters('product_type_selector', array(
		'simple' => __('Sản phẩm', 'woocommerce'),
		'grouped' => __('Sản phẩm nhóm', 'woocommerce'),
		'external' => __('Sản phẩm liên kết', 'woocommerce')
	), $product_type);

	$type_box  = '<label for="product-type"><select id="product-type" name="product-type"><optgroup label="' . __('Loại sản phẩm', 'woocommerce') . '">';
	...
	$product_type_options = apply_filters('product_type_options', array(
		'virtual' => array(
			...
			'label' => __('Sản phẩm ảo', 'woocommerce'),
			'description' => __('Sản phẩm ảo là vô hình và không có vận chuyển.', 'woocommerce')
		),
		'downloadable' => array(
			'id' => '_downloadable',
			'wrapper_class' => 'show_if_simple',
			'label' => __('Sản phẩm download', 'woocommerce'),
			'description' => __('Sản phẩm cho phép download về khi mua hàng.', 'woocommerce')
		)
	) );
	...
	<div class="panel-wrap product_data">
		...
		<ul class="product_data_tabs wc-tabs" style="display:none;">
			...
			<li class="active general_options hide_if_grouped"><a href="#general_product_data"><?php _e('Thông tin chung', 'woocommerce'); ?></a></li>

			<li class="tax_tab show_if_simple show_if_variable tax_options"><a href="#tax_product_data"><?php _e('Thuế', 'woocommerce'); ?></a></li>

			<li class="inventory_tab show_if_simple show_if_variable show_if_grouped inventory_options"><a href="#inventory_product_data"><?php _e('QL kho', 'woocommerce'); ?></a></li>

			<li class="shipping_tab hide_if_virtual shipping_options hide_if_grouped hide_if_external"><a href="#shipping_product_data"><?php _e('Giao hàng', 'woocommerce'); ?></a></li>

			<li class="related_product_tab related_product_options"><a href="#related_product_data"><?php _e('Sản phẩm liên quan', 'woocommerce'); ?></a></li>

			<li class="attributes_tab attribute_options"><a href="#woocommerce_attributes"><?php _e('Thuộc tính', 'woocommerce'); ?></a></li>

			<li class="advanced_tab advanced_options"><a href="#advanced_product_data"><?php _e('Nâng cao', 'woocommerce'); ?></a></li>
		...
		<div id="general_product_data" class="panel woocommerce_options_panel"><?php
			...
			echo '<div class="options_group show_if_simple show_if_variable show_if_external">';
				...
				if( get_option('woocommerce_enable_sku', true) !== 'no' ) :
					woocommerce_wp_text_input( array( 'id' => '_sku', 'label' => '<abbr title="'. __('Đơn vị lưu trữ kho', 'woocommerce') .'">' . __('Mã kho', 'woocommerce') . '</abbr>', 'desc_tip' => 'true', 'description' => __('SKU refers to a Stock-keeping unit, a unique identifier for each distinct product and service that can be purchased.', 'woocommerce') ) );
				else:
			...
			echo '<div class="options_group show_if_external">';
				...
				// External URL
				woocommerce_wp_text_input( array( 'id' => '_product_url', 'label' => __('URL sản phẩm', 'woocommerce'), 'placeholder' => 'http://', 'description' => __('Enter the external URL to the product.', 'woocommerce') ) );
			...
			echo '<div class="options_group pricing show_if_simple show_if_external">';
				...
				// Price
				woocommerce_wp_text_input( array( 'id' => '_regular_price', 'class' => 'wc_input_price short', 'label' => __('Giá', 'woocommerce') . ' ('.get_woocommerce_currency_symbol().')' ) );

				// Special Price
				woocommerce_wp_text_input( array( 'id' => '_sale_price', 'class' => 'wc_input_price short', 'label' => __('Giá khuyến mãi', 'woocommerce') . ' ('.get_woocommerce_currency_symbol().')', 'description' => '<a href="#" class="sale_schedule">' . __('Thời gian khuyến mãi', 'woocommerce') . '</a>' ) );

				// Special Price date range
				$field = array( 'id' => '_sale_price_dates', 'label' => __('Ngày bán khuyến mãi', 'woocommerce') );
			...
			echo '<div class="options_group hide_if_virtual hide_if_grouped">';
				...
				if( get_option('woocommerce_enable_weight', true) !== 'no' ) :
					woocommerce_wp_text_input( array( 'id' => '_weight', 'label' => __('Nặng', 'woocommerce') . ' ('.get_option('woocommerce_weight_unit').')', 'placeholder' => '0.00', 'description' => __('Weight in decimal form', 'woocommerce') ) );
				else:
				...
				if( get_option('woocommerce_enable_dimensions', true) !== 'no' ) :
					?><p class="form-field dimensions_field">
						<label for"product_length"><?php echo __('Kích thước', 'woocommerce') . ' ('.get_option('woocommerce_dimension_unit').')'; ?></label>
						<span class="wrap">
							<input id="product_length" placeholder="<?php _e('Dài (L)', 'woocommerce'); ?>" class="input-text" size="6" type="text" name="_length" value="<?php echo get_post_meta( $thepostid, '_length', true ); ?>" />
							<input placeholder="<?php _e('Rộng (W)', 'woocommerce'); ?>" class="input-text" size="6" type="text" name="_width" value="<?php echo get_post_meta( $thepostid, '_width', true ); ?>" />
							<input placeholder="<?php _e('Cao (H)', 'woocommerce'); ?>" class="input-text last" size="6" type="text" name="_height" value="<?php echo get_post_meta( $thepostid, '_height', true ); ?>" />
						</span>
		...
		<div id="tax_product_data" class="panel woocommerce_options_panel">
			...
			// Tax
			woocommerce_wp_select( array( 'id' => '_tax_status', 'label' => __('Trạng thái', 'woocommerce'), 'options' => array(
		...
		<div id="inventory_product_data" class="panel woocommerce_options_panel">
			...
			if (get_option('woocommerce_manage_stock')=='yes') {
				...
				// manage stock
				woocommerce_wp_checkbox( array( 'id' => '_manage_stock', 'wrapper_class' => 'show_if_simple show_if_variable', 'label' => __('QL kho?', 'woocommerce') ) );
				...
				// Stock
				woocommerce_wp_text_input( array( 'id' => '_stock', 'label' => __('Số lượng còn', 'woocommerce'), 'desc_tip' => true, 'description' => __('Stock quantity. If this is a variable product this value will be used to control stock for all variations, unless you define stock at variation level.', 'woocommerce') ) );
			...
			// Stock status
			woocommerce_wp_select( array( 'id' => '_stock_status', 'label' => __('Trạng thái kho', 'woocommerce'), 'options' => array(
			...
			if (get_option('woocommerce_manage_stock')=='yes') {
				...
				// Backorders?
				woocommerce_wp_select( array( 'id' => '_backorders', 'label' => __('Cho phép đặt hàng?', 'woocommerce'), 'options' => array(
					'no' => __('Không cho phép', 'woocommerce'),
					'notify' => __('Cho phép khi thông báo khách hàng', 'woocommerce'),
					'yes' => __('Cho phép', 'woocommerce')
