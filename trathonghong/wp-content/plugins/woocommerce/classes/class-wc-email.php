...
class WC_Email {
	...
	function customer_processing_order( $order_id ) {
		...
		$email_heading = __('Chấp nhận đơn hàng', 'woocommerce');

		$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);

		$subject = apply_filters( 'woocommerce_email_subject_customer_procesing_order', sprintf( __( '[%s] chấp nhận đơn hàng', 'woocommerce' ), $blogname ), $order );
