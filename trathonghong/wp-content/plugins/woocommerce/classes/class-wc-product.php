...
class WC_Product {
			...
			if ($this->is_in_stock()) :
				if ( $this->get_total_stock() > 0 ) :
					...
					switch ( $format_option ) {
						case 'no_amount' :
							$format = __('Còn hàng', 'woocommerce');
						break;
						case 'low_amount' :
							$low_amount = get_option( 'woocommerce_notify_low_stock_amount' );

							$format = ( $this->get_total_stock() <= $low_amount ) ? __('Chỉ còn %s sản phẩm', 'woocommerce') : __('Còn hàng', 'woocommerce');
						break;
						default :
							$format = __('%s còn hàng', 'woocommerce');
						break;
					}
					...
					if ($this->backorders_allowed() && $this->backorders_require_notification()) :
						$availability .= ' ' . __('(cho phép đặt hàng)', 'woocommerce');
					endif;
	...
	function is_on_sale() {
		if ($this->has_child()) :
			...
			foreach ($this->get_children() as $child_id) :
				...
				if ( $sale_price!="" && $sale_price >= 0 && $sale_price<$regular_price) return true;
			endforeach;

		else :

			if ( $this->sale_price && $this->sale_price==$this->price && $this->sale_price<$this->regular_price) return true;
		...
		elseif ($this->is_type('variable')) :
			...
			elseif ($this->price == 0 ) :
				...
				if ($this->is_on_sale() && isset($this->regular_price)) :
					...
					$price .= $this->get_price_html_from_to( $this->regular_price, __('Miễn phí!', 'woocommerce') );

					$price = apply_filters('woocommerce_free_sale_price_html', $price, $this);

				else :

					$price = __('Miễn phí!', 'woocommerce');
	...
	function get_price_html_from_text() {
		return '<span class="from">' . _x('Từ:', 'min_price', 'woocommerce') . ' </span>';
	}
	...
	function get_rating_html( $location = '' ) {
		...
		if ( $average_rating>0 ) :
			return '<div class="star-rating" title="'.sprintf(__('Đánh giá %s / 5', 'woocommerce'), $average_rating).'"><span style="width:'.($average_rating*$star_size).'px"><span class="rating">'.$average_rating.'</span> '.__('Vượt quá 5', 'woocommerce').'</span></div>';
		else :
			return '';
		endif;
	}
