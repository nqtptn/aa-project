							...
							<?php
								...
								echo $woocommerce->cart->get_item_data( $values );
                   				...
                   				if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $values['quantity'] ) )
                   					echo '<p class="backorder_notification">' . __('Cho phép đặt hàng', 'woocommerce') . '</p>';
							?>
						</td>
			...
			<td colspan="6" class="actions">
				...
				<?php if ( get_option( 'woocommerce_enable_coupons' ) == 'yes' && get_option( 'woocommerce_enable_coupon_form_on_cart' ) == 'yes') { ?>
					<div class="coupon">
						...
						<label for="coupon_code"><?php _e('Coupon', 'woocommerce'); ?>:</label> <input name="coupon_code" class="input-text" id="coupon_code" value="" /> <input type="submit" class="button" name="apply_coupon" value="<?php _e('Nhập phiếu giảm giá', 'woocommerce'); ?>" />
				...
				<input type="submit" class="button" name="update_cart" value="<?php _e('Cập nhật giỏ hàng', 'woocommerce'); ?>" /> <input type="submit" class="checkout-button button alt" name="proceed" value="<?php _e('Thực hiện đặt hàng &rarr;', 'woocommerce'); ?>" />
