...
?>
	<table class="shop_table my_account_orders">
		...
		<thead>
			<tr>
				<th class="order-number"><span class="nobr"><?php _e('Đơn hàng', 'woocommerce'); ?></span></th>
				<th class="order-shipto"><span class="nobr"><?php _e('Giao hàng', 'woocommerce'); ?></span></th>
				<th class="order-total"><span class="nobr"><?php _e('Tổng', 'woocommerce'); ?></span></th>
				<th class="order-status" colspan="2"><span class="nobr"><?php _e('Trạng thái', 'woocommerce'); ?></span></th>
			</tr>
		</thead>
				...
				?><tr class="order">
					...
					<td class="order-status" style="text-align:left; white-space:nowrap;">
						...
						<?php if (in_array($order->status, array('pending', 'failed'))) : ?>
							<a href="<?php echo esc_url( $order->get_cancel_order_url() ); ?>" class="cancel" title="<?php _e('Nhấp vào để huỷ đơn hàng', 'woocommerce'); ?>">(<?php _e('Huỷ', 'woocommerce'); ?>)</a>
						<?php endif; ?>
					</td>
					<td class="order-actions" style="text-align:right; white-space:nowrap;">

						<?php if (in_array($order->status, array('pending', 'failed'))) : ?>
							<a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" class="button pay"><?php _e('Đặt hàng', 'woocommerce'); ?></a>
						<?php endif; ?>

						<a href="<?php echo esc_url( add_query_arg('order', $order->id, get_permalink(woocommerce_get_page_id('view_order'))) ); ?>" class="button"><?php _e('Xem', 'woocommerce'); ?></a>
