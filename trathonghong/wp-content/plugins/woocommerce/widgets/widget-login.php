...
class WooCommerce_Widget_Login extends WP_Widget {
	...
	function WooCommerce_Widget_Login() {
		...
		$this->woo_widget_description = __( '', 'woocommerce' );
	...
	function widget( $args, $instance ) {
		...
		if (is_user_logged_in()) {
			...
			$links = apply_filters( 'woocommerce_login_widget_logged_in_links', array(
				__('Xem tài khoản', 'woocommerce') 	=> get_permalink(woocommerce_get_page_id('myaccount')),
				__('Thay đổi password', 'woocommerce') => get_permalink(woocommerce_get_page_id('change_password')),
				__('Thoát', 'woocommerce')		=> wp_logout_url(home_url())
			));
			...
			// Get redirect URL
			$redirect_to = apply_filters( 'woocommerce_login_widget_redirect', get_permalink(woocommerce_get_page_id('cart')) );
			?>
			<form method="post">

				<p><label for="user_login"><?php _e('Tài khoản hoặc E-mail', 'woocommerce'); ?></label> <br/> <input name="log" value="<?php if (isset($_POST['log'])) echo esc_attr(stripslashes($_POST['log'])); ?>" class="input-text" id="user_login" type="text" /></p>

				<p><label for="user_pass"><?php _e('Password', 'woocommerce'); ?></label> <br/> <input name="pwd" class="input-text" id="user_pass" type="password" /></p>

				<p><input type="submit" class="submitbutton" name="wp-submit" id="wp-submit" value="<?php _e('Đăng nhập &rarr;', 'woocommerce'); ?>" /> <br/><a href="<?php echo wp_lostpassword_url(); ?>"><?php echo __('Quên password?', 'woocommerce'); ?></a></p>
