...
<body class="login">
<?php   if ( !is_multisite() ) { ?>
<div id="login"><h1><a href="<?php echo esc_url( apply_filters('login_headerurl', 'http://trathonghong.com/') ); ?>" title="<?php echo esc_attr( apply_filters('login_headertitle', __( 'Trà Thông Hồng' ) ) ); ?>"><?php bloginfo('name'); ?></a></h1>
<?php   } else { ?>
...
function login_footer($input_id = '') {
	?>
	<p id="backtoblog"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php esc_attr_e( 'Bạn đã quên?' ); ?>"><?php printf( __( '&larr; Trở về %s' ), get_bloginfo( 'title', 'display' ) ); ?></a></p>
	</div>
...
function retrieve_password() {
	...
	if ( empty( $_POST['user_login'] ) ) {
		$errors->add('empty_username', __('<strong>Lỗi</strong>: Vui lòng nhập tên đăng nhập hoặc e-mail.'));
	} else if ( strpos( $_POST['user_login'], '@' ) ) {
		$user_data = get_user_by( 'email', trim( $_POST['user_login'] ) );
		if ( empty( $user_data ) )
			$errors->add('invalid_email', __('<strong>Lỗi</strong>: Không có tên đăng nhập liên quan e-mail này!.'));
	} else {
	...
	if ( !$user_data ) {
		$errors->add('invalidcombo', __('<strong>Lỗi</strong>: Tên đăng nhập hoặc e-mail không hợp lệ.'));
		return $errors;
	}
	...
	if ( ! $allow )
		return new WP_Error('no_password_reset', __('Khởi tạo password không cho phép với tên đăng nhập này!'));
...
function register_new_user( $user_login, $user_email ) {
	...
	if ( $sanitized_user_login == '' ) {
		$errors->add( 'empty_username', __( '<strong>Lỗi</strong>: Vui lòng nhập tên đăng nhập.' ) );
	} elseif ( ! validate_username( $user_login ) ) {
		$errors->add( 'invalid_username', __( '<strong>Lỗi</strong>: Tên đăng nhập không hợp lệ, vui lòng nhập tên khác.' ) );
		$sanitized_user_login = '';
	} elseif ( username_exists( $sanitized_user_login ) ) {
		$errors->add( 'username_exists', __( '<strong>ERROR</strong>: Tên đăng nhập đã tồn tại, vui lòng nhập tên khác.' ) );
	}

	// Check the e-mail address
	if ( $user_email == '' ) {
		$errors->add( 'empty_email', __( '<strong>ERROR</strong>: Vui lòng nhập e-mail của bạn.' ) );
	} elseif ( ! is_email( $user_email ) ) {
		$errors->add( 'invalid_email', __( '<strong>ERROR</strong>: E-mail không đúng.' ) );
		$user_email = '';
	} elseif ( email_exists( $user_email ) ) {
		$errors->add( 'email_exists', __( '<strong>ERROR</strong>: E-mail đã được đăng ký, vui lòng nhập e-mail khác.' ) );
	}
...
case 'retrievepassword' :
	...
	login_header(__('Lost Password'), '<p class="message">' . __('Vui lòng nhập tài khoản hoặc địa chỉ email. Bạn sẽ nhận được đường dẫn để tạo lại password qua email.') . '</p>', $errors);
...
<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( site_url( 'wp-login.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
	<p>
		<label for="user_login" ><?php _e('Tên đăng nhập / E-mail:') ?><br />
...
<?php do_action('lostpassword_form'); ?>
	...
	<p class="submit"><input type="submit" name="wp-submit" id="wp-submit" class="button-primary" value="<?php esc_attr_e('Tạo password mới'); ?>" tabindex="100" /></p>
</form>
...
<?php		exit;
	...
	elseif	( isset($_GET['registration']) && 'disabled' == $_GET['registration'] )
		$errors->add('registerdisabled', __('Hiện tại không cho phép đăng ký.'));
	elseif	( isset($_GET['checkemail']) && 'confirm' == $_GET['checkemail'] )
		$errors->add('confirm', __('Kiểm tra e-mail và xác nhận theo đường dẫn trên mail.'), 'message');
	elseif	( isset($_GET['checkemail']) && 'newpass' == $_GET['checkemail'] )
		$errors->add('newpass', __('Kiểm tra e-mail để nhận password mới.'), 'message');
	elseif	( isset($_GET['checkemail']) && 'registered' == $_GET['checkemail'] )
		$errors->add('registered', __('Đăng ký hoàn tất, vui lòng kiểm tra e-mail.'), 'message');
...
<a href="<?php echo esc_url( site_url( 'wp-login.php?action=register', 'login' ) ); ?>"><?php _e( 'Register' ); ?></a> |
<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>" title="<?php esc_attr_e( 'Password Lost and Found' ); ?>"><?php _e( 'Bạn đã quên password?' ); ?></a>
<?php else : ?>
<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>" title="<?php esc_attr_e( 'Password Lost and Found' ); ?>"><?php _e( 'Bạn đã quên password?' ); ?></a>
<?php endif; ?>
</p>
