...
EditableGrid.prototype.selected_row_id = "";
EditableGrid.prototype.selected_row_classname = "selected_row";
	...
	with(this) {
		if (typeof table != "undefined" && table != null) {
			...
			table.onclick = function (e) {
				this.editablegrid.change_forcus_row(e)
			}
		} else {
			if (!_$(containerid)) {
...
EditableGrid.prototype.change_forcus_row = function (e) {
	e = e || window.event;
	with(this) {
		var target = e.target || e.srcElement;
		while (target) {
		...
		if(this.selected_row_id!=""){
			document.getElementById(this.selected_row_id).className="";
		}
		if(this.selected_row_id != target.id){
			target.className=this.selected_row_classname;
			this.selected_row_id = target.id;
		}else{
			this.selected_row_id="";
		}
	}
};
EditableGrid.prototype.change_forcus_row2 = function (e) {
	e = e || window.event;
	with(this) {
		target=e;
		while (target) {
...
EditableGrid.prototype.edit_e = function (e) {
	this.change_forcus_row2(e);
	with(this) {
		var target = e;
		while (target) {
			if (target.tagName == "A" || target.tagName == "TD" || target.tagName == "TH") {
...
EditableGrid.prototype.find_next_editable_cell = function (e,key) {
	var test_e;
	while (e) {
		if (e.tagName == "A" || e.tagName == "TD" || e.tagName == "TH") {
			break
		} else {
			e = e.parentNode;
		}
	}
	if (!e || !e.parentNode || !e.parentNode.parentNode || (e.parentNode.parentNode.tagName != "TBODY" && e.parentNode.parentNode.tagName != "THEAD") || e.isEditing) {
		return
	}
	if (e.tagName == "A") {
		return
	}
	var rowIndex = this.getRowIndex(e.parentNode);
	var count_child = e.parentNode.parentNode.childNodes.length;
	var columnIndex = e.columnIndex;
	if(key==9 || key==39){
		if(columnIndex==(this.columns.length-1) && rowIndex < count_child-1){
			test_e = e.parentNode.nextSibling;
			test_e = test_e.firstChild;
		}else{
			test_e = e.nextSibling;
		}

	}else if(key==37){
		if(rowIndex>0){
			if(columnIndex==0){
				test_e = e.parentNode.previousSibling;
				test_e = test_e.lastChild;
			}else{
				test_e = e.previousSibling;
			}
		}else{
			test_e = e.previousSibling;
		}
		...
		if(rowIndex>0){
			test_e = e.parentNode.previousSibling;
			test_e = test_e.childNodes[columnIndex];
		}else{
			test_e=e;
		}
	}else if(key==40){
		if(rowIndex < count_child-1){
			test_e = e.parentNode.nextSibling;
			test_e = test_e.childNodes[columnIndex];
		}else{
			test_e=e;
	...
	if(test_e!=null && test_e!=undefined){
		var column2 = this.columns[test_e.columnIndex];
		if (!column2.editable) {
			this.find_next_editable_cell(test_e,key);
		}else{
			this.edit_e(test_e);
		}
	}
...
CellEditor.prototype.edit = function (e, b, a, d) {
	...
	c.element = a;
	c.celleditor = this;
	var this_parent=this.editablegrid;
	c.onkeydown = function (f) {
		f = f || window.event;
		if (f.keyCode == 13 || f.keyCode == 9 || f.keyCode == 37 || f.keyCode == 38 || f.keyCode == 39 || f.keyCode == 40) {
			this.onblur = null;
			this.celleditor.applyEditing(this.element, this.celleditor.getEditorValue(this));
			if(f.keyCode == 9 || (f.keyCode >=37 && f.keyCode<=40)){
				this_parent.find_next_editable_cell(a,f.keyCode);
			}
			return false
		}
		if (f.keyCode == 27) {
...
EditableGrid.prototype.updatePaginator = function(){
	...
	// "first" link
	var link = $("<a>").html("<img src='" + this.baseUrl + "/images/gofirst.png'/>&nbsp;");
	if (!this.canGoBack()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	else link.css("cursor", "pointer").click(function(event) { editableGrid.firstPage(); });
	paginator.append(link);
	// "prev" link
	link = $("<a>").html("<img src='" + this.baseUrl + "/images/prev.png'/>&nbsp;");
	if (!this.canGoBack()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	...
	// "next" link
	link = $("<a>").html("<img src='" + this.baseUrl + "/images/next.png'/>&nbsp;");
	if (!this.canGoForward()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	else link.css("cursor", "pointer").click(function(event) { editableGrid.nextPage(); });
	paginator.append(link);
	// "last" link
	link = $("<a>").html("<img src='" + this.baseUrl + "/images/golast.png'/>&nbsp;");
	if (!this.canGoForward()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
