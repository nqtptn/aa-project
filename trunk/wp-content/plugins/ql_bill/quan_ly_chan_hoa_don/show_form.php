...
function post_data(xml_link){
	...
	input_value['id'] = $("#ma_tinh_di").val();
	input_value['giam_doc'] = $("#giam_doc").val();
	input_value['ke_toan_truong'] = $("#ke_toan_truong").val();
	input_value['ke_toan_cong_no'] = $("#ke_toan_cong_no").val();
	input_value['ke_toan_du_lieu'] = $("#ke_toan_du_lieu").val();
	input_value['lien_he'] = $("#lien_he").val();
	if(input_value['id']==""){
		...
		$.ajax({
			...
			dataType: "html",
			data: {
				ma_tinh_di : input_value['id'],
				giam_doc : input_value['giam_doc'],
				ke_toan_truong : input_value['ke_toan_truong'],
				ke_toan_cong_no : input_value['ke_toan_cong_no'],
				ke_toan_du_lieu : input_value['ke_toan_du_lieu'],
				lien_he : input_value['lien_he'],
				action: "add_record",
			},
...
{
	var link='<? echo get_admin_url()?>admin.php?page=quan_ly_chan_hoa_don&noheader=1&nofooter=1&action=XML';
	return link;
}
function get_report_link()
{
	var link='<? echo get_admin_url()?>admin.php?page=quan_ly_chan_hoa_don&noheader=1&nofooter=1&action=export_function';
	return link;
}
...
var update_url="<? echo get_admin_url()?>admin.php?page=quan_ly_chan_hoa_don&action=update_record&noheader=1&nofooter=1";
</script>
<br/>
...
<div id='posting' style='margin-top:10px;border: 1px solid #ccc;-moz-border-radius: 5px;-webkit-border-radius: 5px;padding:10px'>
	...
	<form name="post_form" method="post" onsubmit='return post_data(update_url)'>
		<table>
			<tr>
				<td>
					<label for="ma_tinh_di">Mã tỉnh</label>
				</td>
				<td>
					<label for="giam_doc">Giám đốc</label>
				</td>				
				<td>
					<label for="ke_toan_truong">Kế toán trưởng</label>
				</td>
				<td>
					<label for="ke_toan_cong_no">Kế toán công nợ</label>
				</td>
				<td>
					<label for="ke_toan_du_lieu">Kế toán dữ liệu</label>
				</td>
				<td>
					<label for="lien_he">ĐT liên hệ</label>
				</td>
			</tr>
			<tr>
				<td>
					<input type='text' size='12' id='ma_tinh_di' value=''  />	
				</td>
				<td>
					<input type='text' size='12' id='giam_doc' value=''  />
				</td>				
				<td>
					<input type='text' size='12' id='ke_toan_truong' value=''  />	
				</td>
				<td>
					<input type='text' size='12' id='ke_toan_cong_no' value=''  />
				</td>	
				<td>
					<input type='text' size='12' id='ke_toan_du_lieu' value=''  />	
				</td>
				<td>
					<input type='text' size='12' id='lien_he' value=''  />
