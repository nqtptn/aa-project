...
function createMarker(point, name, address, homeAddress, description, url, email, hours, phone, image,tags) { 
  ...
  if (homeAddress.split(" ").join("")!="") {
    var html = '<div id="sl_info_bubble"><!--tr><td--><strong>' + name + '</strong><br>' + street + street2 + city + state_zip + '<br/> <a href="http://' + slplus.map_domain + '/maps?saddr=' + encodeURIComponent(homeAddress) + '&daddr=' + encodeURIComponent(address) + '" target="_blank" class="storelocatorlink">Dẫn đường</a> ' + more_html + '<br/><!--/td></tr--></div>'; 
  } else {
      ...
      var html = '<center><table width="96%" cellpadding="4px" cellspacing="0" class="searchResultsTable">' +
                 '<tr>' +
                    ...
                    '<td class="results_row_right_column">' + 
                        ...
                        '" target="_blank" class="storelocatorlink">Dẫn đường</a>'+
                        taginfo +
                        '</td>' +
                        '</tr></table></center>'; 
      div.innerHTML = html;
