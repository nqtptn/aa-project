...
class Spreadsheet_Excel_Reader {
	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$out = "<table class=\"$table_class\" style='border-collapse:collapse;border:1px solid #999' border='1'>";
		if ($col_letters) {
			$out .= "<thead>\n\t<tr>";
			if ($row_numbers) {
				$out .= "\n\t\t<th>&nbsp</th>";
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			for ($i=0;$i<=$this->colcount($sheet);$i++){
				$tess_value=$val = $this->val($row,$i,$sheet);
				if(!empty($tess_value)){
					$show_tr=true;
				}
			}
...
if($_GET['action']=="xacnhan"){
	...
	$wpdb->query("DELETE FROM gia_van_don where ma_van_don in (SELECT ma_van_don FROM gia_van_don_import where ma_tinh_di='$ma_tinh_di')");
	if($data2 = $wpdb->query("INSERT INTO gia_van_don(ma_van_don,ten_khach_hang,ten_nguoi_nhan,noi_gui,noi_nhan,thoi_gian_nhan
		,trang_thai,ghi_chu,ngay_tao,ma_tinh_di)
	(SELECT 
		ma_van_don,
		ten_khach_hang,
		ten_nguoi_nhan,
		...
		ngay_tao,
		ma_tinh_di
		FROM gia_van_don_import where ma_tinh_di='$ma_tinh_di')")){
		echo "ok";
		$import_query = array('ma_tinh_di' => $ma_tinh_di);
		$wpdb->delete("gia_van_don_import", $import_query );
	}else{
		echo "error";
	}
}else{
			...
			foreach ($arr_rows as $row) {
				if($i>0){
					$import_query = array(
										...
										'ten_nguoi_nhan' => $row[3],
										'noi_gui' =>  $row[4],
										'noi_nhan' =>  $row[5],
										'thoi_gian_nhan' =>  $row[6],
										'trang_thai' =>  $row[7],
										'ma_tinh_di' =>  $ma_tinh_di,
										'ghi_chu' =>  $row[8]
										);
					$wpdb->insert("gia_van_don_import", $import_query );
				}
				$i++;
			}
			$html_update = "<div class='updated'>Dữ liệu đã được nhập từ file thành công.</div>";
		} // end of 'if arr_rows is array'
		else {
			$html_update = "<div class='updated' style='color: red'>File định dạng không đúng, vui lòng kiểm tra file định dạng.</div>";
		}
	}
?>
...
function xac_nhan()
{
	$.ajax({
		...
		{
			if(response!="error"){
				alert("Đã thực hiện upload vận đơn thành công.");
				$("#noidung").html("");
			}else{
				alert("Có lỗi xảy ra");
			}
...
var update_url="<? echo get_admin_url()?>admin.php?page=xls_import_van_don&action=xacnhan&noheader=1&nofooter=1";
</script>
	<div class="wrap">
		<?php echo $html_update; ?>
		<div id="icon-users" class="icon32"><br /></div>
		<h2>Upload vận đơn</h2>
		<p>Chọn file excel (.xls):</p>
		<form action="admin.php?page=xls_import_van_don" method="post" enctype="multipart/form-data">
			<input type="hidden" name="mode" value="submit">
			<input type="file" name="csv_file" />
			<input type="submit" value="   Import   " />
		</form>
		<p>File excel (.xls) phải được định dạng như sau:</p>
		Số vận đơn, Tên khách hàng, Tên người nhận, Nơi gửi, Nơi nhận, Thời gian nhận, Trạng thái, Ghi chú
		<?
		if($show_value){
			echo "<div id='noidung'><br/>Nội dung file excel (.xls):".$show_value."";
			echo "<br /><input type='button' value='Xác nhận upload vận đơn' onclick='xac_nhan()' /></div>";
		}
		?>
	</div>
<?
